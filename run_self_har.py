...
def get_parser():
    ...
    parser.add_argument('--previous_model_path', default='', type=str,
                        help='path to the previously trained model for fine-tuning')

    return parser


def prepare_dataset(dataset_paths, window_size, get_train_test_users, validation_split_proportion=0.1, verbose=1):
    all_user_datasets = {}
    all_label_lists = []
    ...
    for dataset_path in dataset_paths:
        ...
        with open(dataset_path, 'rb') as f:
            ...
            label_list = dataset_dict.get('label_list', [])
            all_label_lists.append(label_list)

            # Combine user datasets
            for user_id, data in user_datasets.items():
                if user_id not in all_user_datasets:
                    all_user_datasets[user_id] = []
                all_user_datasets[user_id].extend(data)

    print(f"Combined User IDs: {list(all_user_datasets.keys())}")

    # Ensure label lists are consistent across datasets
    combined_label_list = sorted(set(sum(all_label_lists, [])))
    label_map = {label: i for i, label in enumerate(combined_label_list)}
    output_shape = len(combined_label_list)

    har_users = list(all_user_datasets.keys())
...
if __name__ == '__main__':
    ...
    prepared_datasets['labelled'] = prepare_dataset(args.labelled_dataset_paths, window_size, get_fixed_split_users,
    ...
    for i, experiment_config in enumerate(experiment_configs):
        ...
        if experiment_type == 'transform_train':
            ...
            multitask_transform_dataset = self_har_utilities.create_individual_transform_dataset(
                prepared_datasets['unlabelled_combined'], transform_funcs_vectorized)
